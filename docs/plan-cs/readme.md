Zde je český překlad souboru `README.md`**README.md** (původně označeného jako _Koklyp — brainstorming record_). Tento dokument slouží jako retrospektivní záznam rozhodovacího procesu a „metodika“ toho, jak projekt dospěl k aktuálnímu plánu verze v1.

### **README.md (Czech Translation)**

# Koklyp — záznam brainstormingu (shrnuti diskuse)
...
- `koklyp-brainstorming-responses.md`[koklyp-brainstorming-responses.md](https://www.google.com/search?q=koklyp-brainstorming-responses.md) — odpovědi uživatele na otevřené otázky.
...
Každý agent vytvořil vlastní soubory `koklyp-architecture.md`, `koklyp-features.md` a `koklyp-brainstorming.md` na základě kódových bází IronClaw/ZeroClaw a inspirací jako OpenClaw či Hermes Agent.
...
- `koklyp-architecture.md`[koklyp-architecture.md](https://www.google.com/search?q=koklyp-architecture.md) — propojil GPT pojmenování 5úrovňové paměti a rámec „MCP-as-sidecar“ s páteří od Claude; převzal od MiniMax preemptivní prořezávání, časovou degradaci (time decay), `tool_search`, catchup execution a tunelování; převzal od Gemini výběr implementací JGit/Fabric8; zamítl MiniMax strukturu modulů KMP a doporučení pro Native; zamítl Gemini hrubé rozvržení do 6 modulů.

- `koklyp-features.md`[koklyp-features.md](https://www.google.com/search?q=koklyp-features.md) — úplný seznam funkcí v1/v2/L sladěný s architekturou.

- `koklyp-brainstorming.md`[koklyp-brainstorming.md](https://www.google.com/search?q=koklyp-brainstorming.md) — pracovní dokument s názory, pushbacky, architektonickými sázkami a otevřenými otázkami, záměrně zaujímající vyhraněné pozice pro vyvolání diskuse s uživatelem.
...
Uživatel odpověděl prostřednictvím souboru `koklyp-brainstorming-responses.md`[koklyp-brainstorming-responses.md](https://www.google.com/search?q=koklyp-brainstorming-responses.md) a uzavřel otevřené otázky:
...
|**LLM provider**|**OpenAI API + BYOK**|Uživatel má interní LLM Gateway; koklyp dodá **jeden** OpenAI-compat klient.|
...
Klíčové zjednodušení: Díky internímu LLM Gateway odpadla nutnost řešit v rámci koklyp nativní adaptéry pro Anthropic, Bedrock či Gemini, a také komplexní fallback řetězce.

## 6. Architektura a funkce (druhý průchod)

Oba hlavní dokumenty (`koklyp-architecture.md` a `koklyp-features.md`) byly aktualizovány:
...
2. `Koklyp Brainstorming.md` (tento soubor) — jak jsme se k plánu dopracovali.
