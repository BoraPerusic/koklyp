# koklyp — specifikace v1 (scope)

Toto je **kontrakt rozsahu (scope contract)** pro koklyp v1. Slouží jako referenční dokument při rozhodování, zda daná funkce patří do aktuální verze či nikoliv ("is this in or out?").
...
Zdroje pro upstream rozhodnutí: [`koklyp-brainstorming-responses.md`](koklyp-brainstorming-responses.md), [`koklyp-architecture.md`](koklyp-architecture.md), [`koklyp-features.md`](koklyp-features.md).

---

## 1. Téze v1 (definice úspěchu)

Jeden uživatel může spustit příkaz `koklyp run` a získá **osobního autonomního agenta**, který:
...
6. Je rozšiřitelný pomocí **PF4J pluginů stahovaných z container registry** bez nutnosti rekompilace koklyp.
...
Pokud může uživatel používat koklyp denně po dobu dvou týdnů, aniž by bylo nutné restartovat JVM, ladit proces nebo ručně upravovat databázi — verze v1 je hotová.
...
- koog zapouzdřený za fasádou `LlmProvider` / `KoklypAgent`.
...
- Trait `MemoryStore` + SQLite backend (`~/.koklyp/koklyp.db`).
- FTS5 + sqlite-vec; Reciprocal Rank Fusion při `k₀ = 60`.
- Markdown workspace (`~/.koklyp/workspace/`) se soubory `MEMORY.md`, `IDENTITY.md`, `HEARTBEAT.md`, `daily/YYYY-MM-DD.md`.
...
- Channel `healthCheck()` dostupný přes `koklyp doctor`.
...
- **MCP server**: vystavení vestavěných nástrojů koklyp pro ostatní agenty.
...
- PF4J `PluginManager` řídící wrapper `koklyp` PluginManageru.
- Modul `plugin-api` vystavující `KoklypPlugin` + `PluginHost` + rozhraní schopností. `koog`, `slack-bolt` (při přidání), JDBC atd. nejsou pro pluginy viditelné — pouze `api` + `plugin-api`.
- Layout pluginu: adresář nebo fat-JAR; `plugin.properties` (PF4J) + `plugin.toml` (koklyp manifest).
...
- Příkazy `koklyp plugin install <oci-ref>`, `koklyp plugin install <path>` (sideload), `koklyp plugin list` a `koklyp plugin remove <name>`.
- Namespacing pluginů pro skilly: `plugin:<plugin>/<skill>`.
- ABI verze fixována v manifestu (`koklyp_api_version`); nekompatibilní pluginy se odmítnou načíst s jasnou chybou.

### 2.9 Skilly

- Markdown balíčky ve formátu agentskills.io pod `~/.koklyp/skills/`.
...
- Tool receipts: Ed25519-signed, hash-chained, append-only na disku v `~/.koklyp/receipts/YYYY-MM.log`.
...
- Nouzové zastavení (`koklyp estop`).
...
- Kontroly `koklyp doctor`: validita konfigurace, dostupnost LLM endpointu, zdraví kanálů, manifesty pluginů, sandbox (ve v1 no-op, detekce přítomnosti), přístup ke keychain.
- `koklyp service install` generuje a instaluje systemd unit / launchctl plist / Windows-Service definici. Daemon mód + PID soubor.
...
- Single fat JAR přes Gradle Shadow (`koklyp.jar`), shell wrapper `./koklyp`.
...
| Latence na turn | < 250 ms na straně koklyp (bez LLM) | OTel span budgety |
| Paměťová stopa (idle) | < 400 MB RSS | `ps` snapshot v `koklyp doctor` |
...
| Načtení pluginu (lokální) | < 500 ms | Benchmark v `koklyp plugin install --dry-run` |
...
1. `./koklyp onboard` provede uživatele nastavením LLM endpointu + Telegramu a vytvoří funkční `~/.koklyp/config.toml`.
2. `./koklyp run` se spustí; `./koklyp doctor` hlásí zelenou pro config / LLM / kanály / keychain / pluginy.
3. Z **CLI** lze vést multi-turn chat, který volá nástroje `file_system.read`, `web_search` a `http`, přičemž potvrzení (receipts) se zapisují na disk a jsou viditelná přes `./koklyp status --recent`.
...
7. **Plugin**: lze publikovat in-tree `hello-world` plugin do lokální OCI registry, poté provést `koklyp plugin install <ref>` a zavolat jeho nástroj z chatu. Stejný plugin načtený s `signature_mode = required` + platným Ed25519 podpisem funguje; bez podpisu se odmítne.
8. **MCP server**: `koklyp mcp serve --stdio` z Claude Desktop / Cursor umožní tomuto klientovi volat nástroj `file_system` koklyp.
...
12. **Potvrzení (Receipts)**: každé volání nástroje je v `~/.koklyp/receipts/YYYY-MM.log`, hash řetězec sedí a `koklyp memory show receipts/2026-04.log --verify` vrací OK.
13. **Služba**: `./koklyp service install --systemd` vygeneruje unit; unit spustí koklyp; `systemctl restart` korektně zastaví a obnoví proces (běžící turny skončí čistě, čekající schválení jsou obnovena).
14. **Estop**: `./koklyp estop` uprostřed volání nástroje zastaví smyčku, nezanechá žádné zombie procesy a log potvrzení zaznamená přerušení.
