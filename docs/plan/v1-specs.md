# koklyp — v1 specs (scope)

This is the **scope contract** for koklyp v1. It is the document we point at when deciding "is this in or out?"
...
Source of upstream decisions: [`koklyp-brainstorming-responses.md`](koklyp-brainstorming-responses.md), [`koklyp-architecture.md`](koklyp-architecture.md), [`koklyp-features.md`](koklyp-features.md).

---

## 1. v1 thesis (what success looks like)

A single human can run `koklyp run` and end up with **a personal autonomous agent** that:
...
6. Can be extended with **PF4J plugins pulled from a container registry** without recompiling koklyp.
...
If a human can use koklyp daily for two weeks without us having to restart it, debug it from the JVM, or hand-edit the database — v1 is done.
...
- koog wrapped behind koklyp's `LlmProvider` / `KoklypAgent` facade.
...
- `MemoryStore` trait + SQLite backend (`~/.koklyp/koklyp.db`).
- FTS5 + sqlite-vec; Reciprocal Rank Fusion at `k₀ = 60`.
- Markdown workspace (`~/.koklyp/workspace/`) with `MEMORY.md`, `IDENTITY.md`, `HEARTBEAT.md`, `daily/YYYY-MM-DD.md`.
...
- Channel `healthCheck()` surfaced by `koklyp doctor`.
...
- **MCP server**: expose koklyp's built-in tools to other agents.
...
- PF4J `PluginManager` driving a `koklyp` PluginManager wrapper.
- `plugin-api` module exposing `KoklypPlugin` + `PluginHost` + capability interfaces. `koog`, `slack-bolt` (when added), JDBC, etc. are *not* visible to plugins — only `api` + `plugin-api`.
- Plugin layout: directory or fat-JAR; `plugin.properties` (PF4J) + `plugin.toml` (koklyp manifest).
...
- `koklyp plugin install <oci-ref>` and `koklyp plugin install <path>` (sideload) and `koklyp plugin list` and `koklyp plugin remove <name>`.
- Plugin namespacing for skills: `plugin:<plugin>/<skill>`.
- ABI version pinned in manifest (`koklyp_api_version`); incompatible plugins refuse to load with a clear error.

### 2.9 Skills

- Markdown bundles in agentskills.io format under `~/.koklyp/skills/`.
...
- Tool receipts: Ed25519-signed, hash-chained, append-only on disk at `~/.koklyp/receipts/YYYY-MM.log`.
...
- `koklyp doctor` checks: config validity, LLM endpoint reachability, channel health, plugin manifests, sandbox (no-op in v1, presence detection), keychain access.
- `koklyp service install` generates and installs systemd unit / launchctl plist / Windows-Service definition. Daemon mode + PID file.
...
- Single fat JAR via Gradle Shadow (`koklyp.jar`), shell wrapper `./koklyp`.
...
| Per-turn latency overhead | < 250 ms koklyp-side (excludes LLM) | OTel span budgets |
| Memory footprint (idle) | < 400 MB RSS | `ps` snapshot in `koklyp doctor` |
...
| Plugin load (cached, local) | < 500 ms | Bench in `koklyp plugin install --dry-run` |
...
1. `./koklyp onboard` walks me through an LLM endpoint + Telegram setup and produces a working `~/.koklyp/config.toml`.
2. `./koklyp run` boots; `./koklyp doctor` reports green for config / LLM / channels / keychain / plugins.
3. From the **CLI**, I can have a multi-turn chat that calls `file_system.read`, `web_search`, and `http` tools, with receipts written to disk and visible via `./koklyp status --recent`.
...
7. **Plugin**: I can publish the in-tree `hello-world` plugin to a local OCI registry, then `koklyp plugin install <ref>`, then call its tool from a chat turn. Same plugin loaded with `signature_mode = required` + a valid Ed25519 signature works; loaded without a signature it refuses.
8. **MCP server**: `koklyp mcp serve --stdio` from Claude Desktop / Cursor lets that client call koklyp's `file_system` tool.
...
12. **Receipts**: every tool call is in `~/.koklyp/receipts/YYYY-MM.log`, the chain hash verifies, and `koklyp memory show receipts/2026-04.log --verify` returns OK.
13. **Service**: `./koklyp service install --systemd` generates a unit; the unit starts koklyp; `systemctl restart` cleanly stops and resumes (in-flight turns terminate cleanly, pending approvals are restored).
14. **Estop**: `./koklyp estop` mid-tool-call halts the loop, leaves no zombie processes, and the receipts log records the abort.
...
| M9 | **Operations** | `koklyp doctor`, `koklyp service install`, `koklyp onboard`, fat-JAR build via Shadow, structured logs, OTel wiring. |
...
(Pulled from `koklyp-brainstorming.md` §4 and updated after the responses.)
