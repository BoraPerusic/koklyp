...
| M0.T6 | `plugin-api` module — `KoklypPlugin`, `PluginHost`, `Capability`, `Permission`, `SecretHandle`, `GatedHttpClient` | M0.T5 | M | Compiles with only `api` + `pf4j` as deps; `KoklypPlugin` extends `org.pf4j.Plugin`. |
...
| M0.T8 | `config` module — TOML schema + loader + validation diagnostics | M0.T5 | L | `KoklypConfig.load(path) → Result<KoklypConfig, ConfigErrors>`; bad config produces row/col-pinpointed errors. |
...
| M0.T11 | Minimum-viable `cli-app` skeleton (clikt-based subcommand parser) | M0.T1 | S | `./koklyp --help` lists all v1 subcommands as stubs; each prints "not yet implemented". |
...
| M1.T4 | `WorkspaceFs` API (read/write/list/append) confined to `~/.koklyp/workspace/` | M0.T8 | M | Bounds violations throw `KoklypException.Security`; `read/write` round-trip; markdown autoinference. |
...
| M2.T13 | `KoklypAgent` facade with `handleMessage(IncomingMessage): HandleOutcome` | M2.T11 | M | All `HandleOutcome` variants wired; `Pending` distinguishable from `NoResponse`. |
...
| M3.T8 | Ed25519 receipts log writer (chain + sig) | M1.T2, M0.T9 | L | NDJSON file at `~/.koklyp/receipts/YYYY-MM.log` per §13 of arch; signing key stored in `secrets.db`. |
| M3.T9 | Receipts verifier (`koklyp memory show receipts/<file> --verify`) | M3.T8 | M | Walks the file, checks chain + signatures; reports first divergence. |
...
| M3.T11 | Emergency stop (`koklyp estop` IPC + dispatcher honors it) | M2.T13 | M | Stop fires inside an in-flight tool call; receipts log records `{aborted: true}`. |
...
**M4 done when:** every v1 tool has a unit test, a receipts entry on success, a `ToolResult.Err` on policy violation, and `koklyp tool list` enumerates them with their risk.
...
| M5.T10 | Channel `healthCheck()` exposed in `/api/status` and `koklyp doctor` | M5.T2, M5.T5, M5.T8 | S | Each channel reports `Up | Degraded | Down`. |
...
| ID | Title | Deps | Size | Acceptance |
|---|---|---|---|---|
| **M6.T1** | **PF4J spike — hello-world plugin loaded from a local JAR** | **M0.T6** | **L** | **A `plugin-template/`-derived hello-world plugin contributes a `say_hello` Tool. `koklyp plugin install <local-path>` loads it; calling `say_hello` from a chat returns the expected output. No manifest validation, no signature, no ACR yet — just the loader path.** |
| M6.T2 | `PluginManagerWrapper` (PF4J `DefaultPluginManager` subclass + classloader rules) | M6.T1 | M | Plugin classloader exposes `api` + `plugin-api` only (verified by negative test: plugin importing `com.koklyp.core.*` fails to load). |
...
| M6.T6 | ABI compatibility check (`koklyp_api_version`) | M6.T3 | S | Incompatible plugin rejected with a clear diagnostic. |
...
| M6.T9 | `koklyp plugin install <oci-ref>` flow (pull → verify → extract → load) | M6.T7, M6.T8 | M | Round-trip: publish hello-world to a local OCI registry (via `oras push`), then install via koklyp; tool callable. |
| M6.T10 | `koklyp plugin install <local-path>` (sideload) | M6.T2 | S | Used by dev workflow; mirrors the OCI path's last steps. |
| M6.T11 | `koklyp plugin list` / `koklyp plugin remove` | M6.T7 | M | List shows status (loaded/error) + capabilities; remove stops + deletes; install records persisted in `settings`. |
...
| M7.T2 | `mcp-server` — expose koklyp tools as MCP server | M2.T6, M7.T1 | L | All `Low|Medium`-risk tools advertised; `High` gated by `expose_high_risk` flag. |
...
**M7 done when:** a sample stdio MCP server (e.g. `@modelcontextprotocol/server-filesystem`) is consumable from a chat turn AND koklyp's `file_system` tool is callable from Claude Desktop.
...
| ID | Title | Deps | Size | Acceptance |
|---|---|---|---|---|
| M9.T1 | `koklyp doctor` — config / LLM / channels / keychain / plugins / sandbox detect | M5.T10, M6.T11 | M | Each check returns `Pass | Warn | Fail` with a remediation hint. |
| M9.T2 | `koklyp service install/start/stop/uninstall` | M0.T11 | M | systemd unit / launchctl plist / Windows-Service definition generated and installed. |
| M9.T3 | Daemon mode + PID file + graceful shutdown | M0.T11 | S | SIGTERM drains turns within a deadline, then exits 0. |
| M9.T4 | Onboarding wizard (`koklyp onboard`) | M0.T8, M5.T8 | L | Walks through LLM endpoint + Telegram + admin password; generates `config.toml` + secrets; deletes `BOOTSTRAP.md`. |
...
| M9.T6 | Fat-JAR build via Gradle Shadow + `./koklyp` shell wrapper | M0.T1 | S | `./gradlew shadowJar` produces a single jar; wrapper runs it. |
| M9.T7 | `koklyp completion bash/zsh/fish` | M0.T11 | S | Shell completion for subcommands. |
| M9.T8 | `koklyp status --recent` | M3.T8 | S | Prints recent receipts, last LLM call, channel health. |

**M9 done when:** a fresh user runs `koklyp onboard` → `koklyp service install` → koklyp comes up under systemd and stays up across a host reboot.
...
6. **M5 (channels)** — CLI first, then web, then Telegram. Each picks up `KoklypAgent` from M2.
...
5. M9.T4 onboarding wizard → minimal `koklyp init` that writes a starter `config.toml`.
