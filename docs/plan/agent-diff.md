...
A line-by-line comparison of the four drafts the agents produced for koklyp's first round of architecture/features/brainstorming. Claude is the reference (because the user picked it as the spine of the synthesis); the other three are compared against it.
...
- `docs/claude/{koklyp-architecture,koklyp-features,koklyp-brainstorming}.md` + `docs/claude/docs/claws/{stack-notes.md,inspirations.md,iron/*,zero/*}`
- `docs/gpt/{koklyp-architecture,koklyp-features,koklyp-brainstorming}.md` + `docs/gpt/docs/{iron,zero}/*`
- `docs/gemini/{koklyp-architecture,koklyp-features,brainstorming}.md` + `docs/gemini/docs/claws/{iron,zero}/architecture.md`
- `docs/minimax/{koklyp-architecture,koklyp-features,koklyp-brainstorming}.md` + `docs/minimax/docs/claws/{iron,zero}/README.md`
...
| **Agent core framework** | Wrap koog behind a koklyp facade | "Verify koog surface; wrap it" | Open: bespoke vs framework | Verify koog multiplatform; if not, build own |
...
|---|---|
| **5-tier memory framing** (Tier 0 Live / Tier 1 Transcript / Tier 2 Curated / Tier 3 Derived / Tier 4 Retrieval) with explicit names | Adopted into `koklyp-architecture.md` §13 as a documentation framing (Claude's draft had the same content but didn't name the tiers). |
...
| **Module-naming convention** (`koklyp-domain`, `koklyp-runtime`, `koklyp-providers-api`, etc.) | Named slightly differently from Claude's `modules/api`, `core/`, `tools/`, `providers/`, but the cuts are the same. We've kept Claude's names; GPT's naming is more verbose. |
...
- **`MemoryStore` as a trait isn't called out as a kernel-ABI extension point** — GPT discusses `koklyp-memory` as a module but not as a pluggable interface in the Hermes sense.
...
|---|---|
| **JSR-223 Kotlin scripting for trusted scripts vs WASM for untrusted** | Conceptually relevant under the new direction (no WASM): we could ship a "trusted Kotlin script tool" via JSR-223 (`KotlinScriptEngine`) for rapid in-tree scripting. Listed in `koklyp-features.md` as a possible follow-up, not v1. |
...
- **Lists 6 modules (`koklyp-core`, `koklyp-providers`, `koklyp-channels`, `koklyp-memory`, `koklyp-tools`, `koklyp-gateway`).** Claude's 21 modules and GPT's 14 are more realistic for a project this size. Gemini's six are too coarse — `koklyp-tools` would have to contain the dispatcher, the WASM runtime, the MCP client, and every built-in tool, which is a maintenance disaster.
...
| **"Time decay on non-Core memories" (H6)** | Listed as v2 feature in `koklyp-features.md` §9. |
...
- **Catchup execution.** Cron jobs that were due during a koklyp downtime: do they run on startup? MiniMax raises this; Claude doesn't.
...
| MCP server (expose koklyp tools) | First-class day one | M1 | Not explicit | Not explicit | First-class day one |
...
Where the four agents converged (§4), commit. Where they diverged (§5) and the user has decided, follow the user. Where they diverged and the user hasn't decided (multi-tenant, SOPs, BYO-key vs default), surface the question in `koklyp-brainstorming.md` and argue for the lean position.
