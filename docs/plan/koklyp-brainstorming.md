# koklyp — brainstorming (synthesis)
...
The koklyp `LlmProvider` trait is *ours*. Internally, `KoogLlmProvider` adapts koog. If we have to swap, one file changes. (Gemini's brainstorming raised the alternative: build our own lightweight loop. I'd push back — that's reinventing the wheel for boring infrastructure. Wrap, don't replace, and keep the wrap thin.)
...
But if you have a concrete use case — "I want koklyp to handle my weekly deploy with explicit approval steps" — then SOPs might earn their place in v1.
...
- **One process per user** — each user runs their own koklyp, federated channels.
...
**My current bias: track versions in the manifest, allow `koklyp plugin install <name>@<version>` and `koklyp plugin rollback <name>` for v2.**
...
14. **NEW: JVM plugin ABI compatibility.** A plugin compiled against `koklyp-api 0.1.0` may break on `0.2.0`. Mitigation: manifest pins `koklyp_api_version`; loader refuses incompatible versions and reports clearly.
...
16. **Memory tier vagueness.** GPT's risk: "memory tiers are described vaguely and implemented inconsistently." Mitigation: the five-tier framing in `koklyp-architecture.md` §13 is now explicit; we hold ourselves to it.
...
- Then the plugin JAR depends only on `koklyp-api`. Clean.
...
**My current bias:** v1 ships shell with policy/validator/receipts only. Subprocess wrapper in v2. Reason: getting `firejail`/`bwrap`/Docker to work cross-platform is real engineering; v1 should focus on the agent loop. Operators who care can wrap koklyp itself in a container.

Pushback wanted: is "shell tool inside an unwrapped JVM" too dangerous a default?

### 6.6 Plugin hot-reload in v1?

Hot-reload (watch `~/.koklyp/plugins/`, reload changed JARs) is a great dev-loop feature. But classloader unload is best-effort, and it adds bug surface.

**My current bias:** v2 only. v1 just rescans on `koklyp plugin install` / restart.
...
- How do we ship the `plugin-api` jar — Maven Central, JitPack, GitHub Packages, or "git submodule the koklyp repo"?
- Do we want the template to bundle a `koklyp test-plugin <jar>` command for round-trip testing?
