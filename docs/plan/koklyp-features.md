...
> **Deltas baked in (after `koklyp-brainstorming-responses.md`):**
> - All WASM features removed. Plugins are JVM JARs loaded via **PF4J**, distributed via OCI container registry (ACR). See `koklyp-architecture.md` §8.
...
| `KoklypPlugin` base class (extends `org.pf4j.Plugin`) — lives in sibling `plugin-api` module | **v1** | Contract for JVM plugin JARs. Methods return `tools/channels/memoryStores/observers`. PF4J handles classloader + lifecycle. |
...
Closed in `koklyp-brainstorming-responses.md` §3.4 + §7: v1 ships **one OpenAI-compatible client, BYOK.** The user's LLM Gateway (which speaks the OpenAI protocol) is the upstream routing layer; koklyp doesn't try to duplicate that responsibility.
...
| Cost tracking per call (token counts → daily $ budget) | **v1** | Informational at the koklyp layer (the gateway is authoritative for billing). |
...
| Provider router (per-hint model selection) | [L] | Gateway concern. Not koklyp's. |
| Fallback chain on transport errors | [L] | Gateway concern. Not koklyp's. |
...
Closed in `koklyp-brainstorming-responses.md` §6.2 (PF4J), §6.4 (signature_mode optional), §6.6 (no hot-reload), §6.7 (OCI/ACR distribution).

| Feature | Tier | Notes |
|---|---|---|
| **PF4J as the plugin framework** | **v1** | `PluginManager`, `Plugin`, `Extension` from PF4J; classloader isolation provided. |
| `plugin-api` module (depends on `api` only) hosting `KoklypPlugin` + `PluginHost` | **v1** | Plugin authors compile against `plugin-api`, not the kernel. |
| Plugin `plugin.properties` (PF4J standard) + `plugin.toml` (koklyp-specific) | **v1** | Properties: id/version/provider/plugin.class. TOML: capabilities/permissions/signature/allowlist_domains/koklyp_api_version. |
...
| `koklyp plugin install <oci-ref>` (e.g. `acr.example.com/koklyp-plugins/linear:0.3.1`) | **v1** | Pull → verify signature → cache → register with PF4J. |
| `koklyp plugin install <local-path>` for local dev | **v1** | Sideloading from a local JAR / directory. |
| Plugin cache at `~/.koklyp/cache/oci/<sha256>/` | **v1** | Content-addressed; safe to delete. |
...
| ABI version pinning per plugin | **v1** | TOML `koklyp_api_version`; refuse incompatible. |
| Hot reload on plugin file change | [L] | Closed in §6.6: not a priority. Plugin updates → restart koklyp. |
...
Closed in `koklyp-brainstorming-responses.md` §1.3 + §7: **v1 ships Web Console + CLI + Telegram. Nothing else.**
...
| Feature | Tier | Notes |
|---|---|---|
| SQLite default at `~/.koklyp/koklyp.db` | **v1** | Closed in §1.8. |
...
| `koklyp sop run/list/validate` | **v2** | |
...
| **`plugin-template/`** Gradle template (Kotlin) | **v1** | `KoklypPlugin` skeleton, sample `Tool`, `plugin.toml`, `plugin.properties`, ACR-publish task. |
...
| Project skeleton generator (`koklyp scaffold tool foo`) | **v2** | |
...
| Architecture doc (`docs/plan/koklyp-architecture.md`) | **v1** | This already exists. |
...
- **Multi-tenant / SaaS.** Out of scope. One koklyp instance per human.
